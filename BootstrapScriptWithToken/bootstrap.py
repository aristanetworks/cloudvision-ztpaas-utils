...
def tryImageUpgrade( e ):
   ...
   cmdList = ["en", "install source {eosUrl} destination flash:/EOS.swi".format(eosUrl=eosUrl)]
   rc, cmdOut = cli.runCommands(cmdList)
   if rc != 0:
      log("Failed to download image, err: {err}".format(err=cmdOut))
      raise Exception("Failed to download swi from {eosUrl}, err: {err}. Aborting.".format(
